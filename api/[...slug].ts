...
function extractSlugFromHostname(hostname: string): string | null {
  ...
  const slugMatch = hostname.match(/^([a-z0-9-]+)-([a-z0-9]+)\.vercel\.app$/i);
  if (!slugMatch) {
    console.log(`extractSlugFromHostname: No match for hostname: ${hostname}`);
    return null;
  }
  ...
  if (
    ...
    console.log(`extractSlugFromHostname: Extracted slug: ${potentialSlug}`);
    return potentialSlug;
  }
  console.log(
    `extractSlugFromHostname: Suffix did not match expected pattern: ${suffix}`
  );
  return null;
}
...
export default async function handler(request: Request) {
  ...
  if (!slug || slug === "") {
    ...
    console.log(
      `No slug in path. Hostname: ${hostname}, Extracted slug: ${extractedSlug}`
    );
    if (extractedSlug) {
      const targetUrl =
      ...
      console.log(
        "No slug in path and could not extract from hostname. Returning 400."
      );
  ...
  try {
    ...
    if (!project || !project.preview_url) {
      ...
      const extractedSlug = extractSlugFromHostname(hostname);
      console.log(
        `Supabase lookup failed. Hostname: ${hostname}, Extracted slug: ${extractedSlug}`
      );
      if (extractedSlug) {
        const targetUrl =
          "https://factory.basedagents.co/" +
